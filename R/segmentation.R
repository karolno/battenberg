...
segment.baf.phased.multisample = function(samplename, inputfile, outputfile, prior_breakpoints_file=NULL, gamma=10, calc_seg_baf_option=3,GENOMEBUILD) {
  ...
  run_pcf = function(BAFrawchr, presegment_chrom_start, presegment_chrom_end, gamma) {
    ...
    # Remove if they do
    recorrect <- FALSE
    if (any(grepl("chr",BAFrawchrseg[,1]))) {
      BAFrawchrseg[,1] <- sub("chr", "", BAFrawchrseg[,1])
      recorrect <-TRUE
    }
    ...
    # KNO
    # # Add chr at the beginning of chr names if they were removed
    if (recorrect) {
  ...
  BAFraw <- Reduce(f = function(...) merge(..., sort = F, all = F), x = lapply(X = inputfile, FUN = read_baf))
