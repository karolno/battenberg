...
callChrXsubclones = function(tumourname,X_gamma=1000,X_kmin=100,genomebuild,AR=TRUE,prior_breakpoints_file=NULL,chrom_names){
  ...
  #####################
  #####################
  ##################### Following comments are by KNO
  #####################
  #####################
  # The whole genome is ~3e9
  # X chrom is ~155e6
  # there should be 90 000 het SNPs if WGS
  # if mutREAD it should be fewer (WGS should give 90000 * 3e9 / 155e6 - 90000 = 1651935 het SNP on autosome)
  # mutREAD has fewer
  # length(which(is.na(match(BAFvals$Chromosome,c("X","chrX")))))
  # so the ratio is length(which(is.na(match(BAFvals$Chromosome,c("X","chrX"))))) / (90000 * 3e9 / 155e6 - 90000)
  # and X chromosome should have length(which(is.na(match(BAFvals$Chromosome,c("X","chrX"))))) / (90000 * 3e9 / 155e6 - 90000) * 90000 het SNPs
  # #####################
  # #####################
